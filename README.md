...
Let's take a look at the first few rows to better understand the raw data:
...
## Clean data 
### Normalize text case by converting all names to uppercase and trimming extra spaces:
...
We clean the age column by:  
1. Identifying the most common valid age (between 0 and 90).  
2. Replacing invalid (<0 or >90) or missing (NULL) ages with this most common value.
   
```sql
SELECT age
FROM club_member_info_cleaned 
WHERE age IS NOT NULL AND age BETWEEN 0 AND 90
GROUP BY age
ORDER BY COUNT(*) DESC
LIMIT 1;
...
WHERE age IS NULL OR age < 0 OR age > 90;
```

### Cleaning the martial_status column by:
1. Finding the most common valid status (ignoring NULL or blank values).
2. Replacing invalid (blank or whitespace) or missing (NULL) statuses with this most common value.

```sql
SELECT martial_status
FROM club_member_info_cleaned
WHERE martial_status IS NOT NULL AND TRIM(martial_status) <> ''
GROUP BY martial_status
ORDER BY COUNT(*) DESC
LIMIT 1;

WITH mode_status AS (
  SELECT martial_status AS val
  FROM club_member_info_cleaned
  WHERE martial_status IS NOT NULL AND TRIM(martial_status) <> ''
  GROUP BY martial_status
  ORDER BY COUNT(*) DESC
...
UPDATE club_member_info_cleaned
SET martial_status = (SELECT val FROM mode_status)
...
```
## After fixing the issues listed above, here is the result after cleaning the data:
...
|ANNALIESE ETOILE|52|married|aetoile2w@artisteer.com||43 Nova Circle,Garden Grove,California|Financial Advisor|10/1/1912|
...
|AJAY DEGENHARDT|53|married|adegenhardtec@chronoengine.com|INVALID|45479 Mayer Street,Newport Beach,California|Help Desk Technician|10/23/2019|
...
|SMITTY BULMER|40|divorced|sbulmergm@addthis.com|INVALID|23370 Forest Dale Street,Pittsburgh,Pennsylvania|VP Marketing|9/25/2017|
...
|FRANCISCO PISER|31|married|fpiserkc@tamu.edu|INVALID|0 Northport Center,Alexandria,Virginia|Marketing Manager|8/4/2012|
...
|MARION HAWLER|55|married|mhawler1p@pinterest.com|INVALID|60701 Crest Line Drive,Fresno,California|Teacher|1/21/2022|
...
|SHIRLINE ESHMADE|41|married|seshmade9h@ed.gov|INVALID|331 Bellgrove Hill,Richmond,Virginia|Quality Engineer|10/8/2016|
...
|CIRILLO BARTLOMIEJCZYK|31|divorced|cbartlomiejczykdj@networksolutions.com|INVALID|4426 Rigney Alley,Henderson,Nevada|Analog Circuit Design manager|9/2/2015|
...
|SUNSHINE DUNBLETON|63|married|sdunbletonee@yellowpages.com|704-231-0109|79497 Milwaukee Point,Charlotte,North Carolina||2/10/2018|
...
|GAIL CUTLER|26|married|gcutlerf2@auda.org.au|INVALID|242 Homewood Junction,Pittsburgh,Pennsylvania|Community Outreach Specialist|11/27/2019|
...
|BLONDIE RICCARDO|66|married|briccardofn@about.com|INVALID|19 Mayer Drive,Chattanooga,Tennessee|Financial Advisor|9/8/2020|
...
|WILFRID DANILOV|63|married|wdanilovix@is.gd|INVALID|5208 Tennyson Road,Dallas,Texas|Account Executive|5/16/2015|
...
|LANNIE CLEMMEY|42|married|lclemmeykj@redcross.org||572 Golf Junction,Mountain View,California|VP Sales|4/8/2016|
...
|SHARLA MACIOCIA|65|single|smaciocian3@adobe.com|INVALID|141 Badeau Plaza,Fresno,California|Staff Accountant IV|6/25/2018|
...
|JOELLA SURR|32|married|jsurrpo@meetup.com|INVALID|775 Boyd Avenue,Houston,Texas|Nurse Practicioner|3/18/2017|
...
|CLAYBORNE PENELLI|23|single|cpenellirf@apple.com|INVALID|01412 Dunning Place,Washington,District of Columbia|Web Developer III|12/24/2021|
